...
process(#ns_revoke_tx{account = AccountPubKey, fee = Fee,
                      name_hash = NameHash, nonce = Nonce} = RevokeTx, Trees0, Height) ->
    ...
    Name1 = aens_names:revoke(RevokeTx, Name0, Height),
...
-spec ttl(revoke_tx()) -> non_neg_integer().
...
assert_ttl(TTL) ->
    case TTL =:= aec_governance:name_revoke_tx_ttl() of
        true ->
            ok;
        false ->
            {error, wrong_ttl}
